...
export function buildAnswer(intent: Intent, message: string, context: Record<string, unknown>): { answer: string; suggestedNextAction: string[] } {
  ...
  switch (intent) {
    ...
    case 'customs_query': {
      ...
      const answer = `当前清关状态：${customs.status}。${customs.reason ?? ''} ${docs} ${order?.channel === 'bonded' ? '保税仓订单清关通常在资料齐全后推进。' : ''}`;
    ...
    case 'logistics_query': {
      ...
      const answer = `当前承运商：${logi.carrier}，预计时效：${logi.etaDays}天。${logi.canDeliverRural ? '可覆盖大部分乡镇，具体以末端网点为准。' : '部分乡镇可能需要自提或二次转运。'} ${customs?.taxHint ?? ''}`;
    ...
    case 'recommendation': {
      ...
      return {
        answer: `给您推荐：${top.join('；')}。推荐理由：更贴合您提到的需求（${reasons}）。温馨提示：不做医疗诊断，首次使用建议先做小范围测试。`,
